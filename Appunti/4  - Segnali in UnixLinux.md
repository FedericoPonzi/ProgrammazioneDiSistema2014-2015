...
Quando si fa il goto non locale, il valore di ritorno non può essere 0 perchè non so se sono tornato lì a seguito di un jump oppure se ho impostato con successo l'etichetta.
    ...
    #include <setjmp.h>
    #
	if((rc= setjmp(jmpbuffer) != 0)  //Ho impostato il punto di ritorno
	{
		//Non entro in questo blocco
		printf("after longjmp: count%d......");
		exit(0);
	}
	printf("rc=%d\n", rc);
	f1(count,val,sum); //chiamo una funzione ed esco dal main.
	
	}
	
	fi(int count int val int sum)
	{
	f2();
	}
	f2(void)
